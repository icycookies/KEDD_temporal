...
from utils.split import kfold_split, cold_drug_split, cold_protein_split, cold_cluster_split, cold_mssl2drug_split, random_split
...
class DDIClassificationDataset(DDIDataset):
    def __init__(self, path, config, split_strategy):
        super(DDIClassificationDataset, self).__init__(path, config, split_strategy)
...
class MSSL2drug(DDIClassificationDataset):
    def __init__(self, path, config, split_strategy):
...
class DrugBank(Dataset):
    ...
    def _load_data(self):
        ...
        """
        for i in range(86):
            if i not in [0, 25, 27, 41, 42, 51, 61]:
                id2label[i] = cnt
                cnt += 1
        """
        with open(os.path.join(self.path, "labeled_triples.csv")) as f:
            ...
            for line in reader:
                if float(line["label"]) == 1:
                    ...
                    label[idx] = 1
                    self.labels.append(label)
        samples = np.sum(np.array(self.labels, dtype=int), axis=0)
        print(samples)
        print("Total ", len(self), " samples")

    def _featurize(self):
        featurizer = SUPPORTED_DRUG_FEATURIZER[self.config["featurizer"]["structure"]["name"]](self.config["featurizer"]["structure"])
