...
from models.task_model.dti_model import SUPPORTED_DTI_NETWORKS
...
def train_dti_net(train_loader, val_loader, model, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        #     eval_dti_net("train", train_loader, model, args, device)
        if val_loader is not None:
            results = eval_dti_net("valid", val_loader, model, args, device)
...
def eval_dti_net(split, val_loader, model, args, device):
    model.mode = 'eval'
    model.eval()
...
def train_dti_other(train_loader, model):
    feats = []
    labels = []
    for mol, prot, label in train_loader:
        feats.append(torch.hstack((mol, prot)))
        labels.append(label)
    feats = torch.vstack(feats).numpy()
    labels = torch.hstack(labels).numpy()
    model.fit(feats, labels)

def eval_dti_other(test_loader, model):
    feats = []
    labels = []
    for mol, prot, label in test_loader:
        feats.append(torch.hstack((mol, prot)))
        labels.append(label)
    feats = torch.vstack(feats).numpy()
...
def main(args, config):
    ...
    # prepare dataset
    if args.dataset in ['yamanishi08', 'bmkg-dti', 'mssl2drug']:
    ...
    if 'train_negatives' not in config['data']: config['data']['train_negatives'] = 1.0
    if 'train_balanced' not in config['data']: config['data']['train_balanced'] = False
    device = torch.device(args.device)

    if args.mode == "train":
        train_dataset = dataset.index_select(dataset.train_index,
                                             balanced=config['data']['train_balanced'],
                                             keep_negatives=config['data']['train_negatives'],
                                             seed=args.seed)
        ...
        train_dataset._build(
            ...
            None if "kg" not in config["data"]["drug"]["modality"] else os.path.join(
                config["data"]["drug"]["featurizer"]["kg"]["save_path"], "dti-" + args.dataset + ".pkl")
        )
        ...
        kge = train_dataset.kge  # same as test_dataset.kge
        if kge is not None:
            kge = {k: torch.from_numpy(kge[k]).to(device) for k in kge}
        if 'network' in config:
            config['network']['kge'] = kge

        if config['model'] in SUPPORTED_DTI_NETWORKS:
            ...
            model = SUPPORTED_DTI_NETWORKS[config['model']](config["network"], pred_dim)
            ...
            model = train_dti_net(train_loader, test_loader, model, args, device)
            results = eval_dti_net("test", test_loader, model, args, device)
        else:
            train_loader = DataLoader(train_dataset, args.batch_size, shuffle=True, num_workers=args.num_workers)
            test_loader = DataLoader(test_dataset, args.batch_size, shuffle=False, num_workers=args.num_workers)
            if config['model'] == 'rf':
                model = RandomForestClassifier(n_estimators=config['n_trees'], max_depth=config['max_depth'],
                                               n_jobs=args.num_workers, random_state=args.seed)
                train_dti_other(train_loader, model)
                results = eval_dti_other(test_loader, model)
        logger.info(", ".join(["%s: %.4lf" % (k, v) for k, v in results.items()]))
        ...
        for i in range(dataset.nfolds):
            ...
            train_dataset = dataset.index_select(dataset.folds[i]["train"],
                                                 ...
                                                 keep_negatives=config['data']['train_negatives'],
                                                 seed=args.seed)
            test_dataset = dataset.index_select(dataset.folds[i]["test"], balanced=False)
            train_dataset._build(
                test_dataset.pair_index,
                None if "kg" not in config["data"]["drug"]["modality"] else os.path.join(
                    config["data"]["drug"]["featurizer"]["kg"]["save_path"],
                    "dti-" + args.dataset + "-" + args.split_strategy + "-fold" + str(i) + ".pkl")
            )
            test_dataset._build(
                test_dataset.pair_index,
                None if "kg" not in config["data"]["drug"]["modality"] else os.path.join(
            ...
            if config['model'] in SUPPORTED_DTI_NETWORKS:
                ...
                logging.info('Number of trainable parameters: ' + str(sum(p.numel() for p in model.parameters())))
                model = train_dti_net(train_loader, test_loader, model, args, device)
                results.append(eval_dti_net("test", test_loader, model, args, device))
            else:
                train_loader = DataLoader(train_dataset, args.batch_size, shuffle=True, num_workers=args.num_workers)
                test_loader = DataLoader(test_dataset, args.batch_size, shuffle=False, num_workers=args.num_workers)
                if config['model'] == 'rf':
                    ...
                    model = SVC()
                train_dti_other(train_loader, model)
                results.append(eval_dti_other(test_loader, model))
...
def add_arguments(parser):
    ...
    parser.add_argument("--output_path", type=str, default="../ckpts/finetune_ckpts/dti/finetune.pth")
