...
from models.task_model.dp_model import DPModel, KEDD4DP
...
def train_dp(train_loader, val_loader, test_loader, model, task, args):
    ...
    for epoch in range(args.epochs):
        ...
        if stopper.step((test_metrics[metric_name]), model, epoch):
            break
...
def main(args, config):
    ...
    if len(config["data"]["drug"]["modality"]) > 1 and config["model"] not in ["molalbef", "biomedgpt"]:
        model = KEDD4DP(config["network"], task_num)
    else:
    ...
    with open(f"{model.name}_{model.model_num}_{args.dataset_name}_test.txt", "a") as f:
        f.write(
            f"{args.dataset_name} test {metric_name}={results[metric_name]} seed {args.seed} best_epoch {epoch} batch_size {args.batch_size} lr {args.lr} weight_decay {args.weight_decay} dropout {config['network']['structure']['drop_ratio']}  ckpts {init_ckpts}\n ")
