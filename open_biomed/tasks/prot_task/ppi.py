...
from models.task_model.ppi_model import KEDD4PPI, PPISeqModel, PPIGraphModel
...
def train_ppi(train_loader, train_network, val_loader, val_network, model, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        if val_loader is not None:
            ...
            if stopper.step(results["F1"], model, epoch):
                break
    if val_loader is not None:
...
def main(args, config):
    ...
    args.deepeik = config['model'] in ['deepeik', 'deepek', 'deepik']
    dataset = SUPPORTED_PPI_DATASETS[args.dataset](args.dataset_path,
                                                   config["data"]["protein"],
                                                   directed=False,
                                                   make_network=args.graph_ppi,
                                                   paired_text=config['model'] in ['deepeik', 'deepik'],
    ...
    if args.mode == "train":
        ...
        train_dataset._build(
            ...
            None if "kg" not in config["data"]["protein"]["modality"] else os.path.join(
                config["data"]["protein"]["featurizer"]["kg"]["save_path"], "ppi-" + args.dataset + ".pkl")
        )
        test_dataset._build(
            test_dataset.pair_index,
            None if "kg" not in config["data"]["protein"]["modality"] else os.path.join(
        ...
        if args.graph_ppi:
            ...
            model = KEDD4PPI(config['network'], train_dataset.num_classes)
            train_network = None
            test_network = None
        else:
