...
class DrugBPEFeaturizer(BaseFeaturizer):
    ...
    def __call__(self, data):
        ...
        input_ids = torch.LongTensor(
            [102] + [i + 30700 for i in result] + [103] + [0] * (self.max_length - 2 - len(result)))
...
class DrugFPFeaturizer(BaseFeaturizer):
    ...
    def __call__(self, data):
        ...
        if mol is not None:
            if self.config['fptype'] == 'path':
                fp = Chem.RDKFingerprint(mol, fpSize=self.config["fpsize"])
            elif self.config['fptype'] == 'morgan':
                fp = AllChem.GetMorganFingerprintAsBitVect(mol, self.config['fpradius'], nBits=self.config["fpsize"])
            else:
                raise ValueError('Invalid fptype: ' + self.config['fptype'])
        ...
        for atom in mol.GetAtoms():
            if self.config["name"] == "ogb":
                atom_feature = [
                    ...
                    safe_index(self.allowable_features['possible_number_radical_e_list'],
...
class DrugMGNNFeaturizer(BaseFeaturizer):
    allowable_atom_list = ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na', 'Ca', 'Fe', 'As', 'Al', 'I', 'B',
